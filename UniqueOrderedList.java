/**
 * Provided Code + Added Iterator Method
 * @author Ivory Huo
 * @param <T>
 */
public class UniqueOrderedList<T extends Comparable<T>> implements UniqueOrderedListADT<T>, SimpleIterable<T> {
//Modified to also implement the SimpleIterable interface (in addition to the UniqueOrderedListADT<T>
	...
	/**
	 * Implementation of the iterator() method from SimpleIterable interface
	 * @return object that implements the ComparableIterator<T> interface 
	 */
    public CopyableIterator<T> iterator() {
        return new UOLIterator<>(head); //Return an iterator starting from the beginning of the list
    }
}
